...
OUT_NPY = OUT_DIR / "ocean_s0_train_1994_2013_norm.npy"
OUT_STATS = OUT_DIR / "ocean_s0_train_1994_2013_stats.json"
...
def pick_var(ds):
    ...
    for name in candidates:
        lname = name.lower()
        if "sal" in lname or "salt" in lname:
            return name
    return candidates[0]
...
def main():
    ...
    if data.ndim != 3:
        raise RuntimeError(f"Expected [T,H,W], got {data.shape} for var {var_name}")
