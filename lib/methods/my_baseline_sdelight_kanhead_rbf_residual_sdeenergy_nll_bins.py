...
class MY_BASELINE_SDELIGHT_KANHEAD_RBF_RESIDUAL_SDEENERGY_NLL_BINS(Base_method):
    def __init__(self, args, device, steps_per_epoch):
        ...
        self.bin_drift = torch.tensor(
            [0.0 if v is None else float(v) for v in data["drift"]],
            dtype=torch.float32,
            device=device
        )
        self.bin_diff2 = torch.tensor(
            [1.0 if v is None else max(float(v), self.sde_eps) for v in data["diffusion2"]],
            dtype=torch.float32,
            device=device
        )
    ...
    def _sde_energy_loss(self, batch_x, pred_y):
        ...
        for t in range(pred_y.shape[1]):
            ...
            drift, diff2 = self._lookup_bins(prev_state)
            resid = delta - drift
