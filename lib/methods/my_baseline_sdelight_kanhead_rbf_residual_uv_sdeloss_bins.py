...
class MY_BASELINE_SDELIGHT_KANHEAD_RBF_RESIDUAL_UV_SDELOSS_BINS(Base_method):
    ...
    def _lookup_single(self, x, edges, drift_table):
        flat = x.reshape(-1)
        idx = torch.bucketize(flat, edges[1:-1])
        drift = drift_table[idx].reshape_as(x)
        return drift

    def _lookup_uv_drift(self, x):
        x_u = x[:, 0:1]
        x_v = x[:, 1:2]

        drift_u = self._lookup_single(x_u, self.bin_edges_u, self.bin_drift_u)
        drift_v = self._lookup_single(x_v, self.bin_edges_v, self.bin_drift_v)

        return torch.cat([drift_u, drift_v], dim=1)
    ...
    def _sde_loss(self, batch_x, pred_y):
        ...
        for t in range(pred_y.shape[1]):
            ...
            drift = self._lookup_uv_drift(prev_state)
            total = total + (delta - drift).pow(2).mean()
