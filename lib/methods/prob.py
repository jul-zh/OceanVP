...
from lib.models import PROB_Model
from lib.utils import reduce_tensor
from .base_method import Base_method


class PROB(Base_method):
    def __init__(self, args, device, steps_per_epoch):
        Base_method.__init__(self, args, device, steps_per_epoch)
        self.model = self._build_model(self.config)
        self.model_optim, self.scheduler, self.by_epoch = self._init_optimizer(steps_per_epoch)

        # нужен для vali/test внутри OceanVP framework
        self.criterion = nn.MSELoss()

    def _build_model(self, args):
        return PROB_Model(**args).to(self.device)
    ...
    def _predict(self, batch_x, batch_y=None, **kwargs):
        # Для vali/test возвращаем только mean prediction
