...
class MY_BASELINE_SDELIGHT_KANHEAD_RBF_RESIDUAL_SDELOSS_BINS_EDGE(Base_method):
    def __init__(self, args, device, steps_per_epoch):
        ...
        self.edge_weight = getattr(args, "edge_weight", 0.5)
    ...
    def _grad_mag(self, x):
        ...
        dx = torch.nn.functional.pad(dx.abs(), (0, 0, 0, 1))
        dy = torch.nn.functional.pad(dy.abs(), (0, 1, 0, 0))
        return dx + dy

    def _edge_weighted_mse(self, pred_y, batch_y):
        grad = self._grad_mag(batch_y)
        g = grad / (grad.mean().detach() + 1e-8)
        w = 1.0 + self.edge_weight * g
        return (w * (pred_y - batch_y).pow(2)).mean()
    ...
    def train_one_epoch(self, runner, train_loader, epoch, num_updates, eta=None, **kwargs):
        ...
        for batch_x, batch_y in train_pbar:
            ...
            with self.amp_autocast():
                ...
                ew_mse = self._edge_weighted_mse(pred_y, batch_y)
                sde_loss = self._sde_loss(batch_x, pred_y)
                loss = ew_mse + self.lambda_sde * sde_loss
            ...
            if self.rank == 0:
                train_pbar.set_description(
                    f"train loss: {loss.item():.4f} | ew_mse: {ew_mse.item():.4f} | sde: {sde_loss.item():.4f} | data time: {data_time_m.avg:.4f}"
                )
            end = time.time()
