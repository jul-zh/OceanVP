...
class MY_BASELINE_SDELIGHT_KANHEAD_RBF_RESIDUAL_SDELOSS_BINS_MIXED03(Base_method):
    def __init__(self, args, device, steps_per_epoch):
        ...
        self.lambda_mae = getattr(args, "lambda_mae", 0.3)
    ...
    def train_one_epoch(self, runner, train_loader, epoch, num_updates, eta=None, **kwargs):
        ...
        for batch_x, batch_y in train_pbar:
            ...
            with self.amp_autocast():
                ...
                mae_loss = self.mae_criterion(pred_y, batch_y)
                sde_loss = self._sde_loss(batch_x, pred_y)
                loss = mse_loss + self.lambda_mae * mae_loss + self.lambda_sde * sde_loss
            ...
            if self.rank == 0:
                train_pbar.set_description(
                    f"train loss: {loss.item():.4f} | mse: {mse_loss.item():.4f} | mae: {mae_loss.item():.4f} | sde: {sde_loss.item():.4f} | data time: {data_time_m.avg:.4f}"
                )
            end = time.time()
