...
class MY_BASELINE_SDELIGHT_KANHEAD_RBF_RESIDUAL_SDELOSS_BINS_SMOOTH(Base_method):
    ...
    def train_one_epoch(self, runner, train_loader, epoch, num_updates, eta=None, **kwargs):
        ...
        for batch_x, batch_y in train_pbar:
            ...
            with self.amp_autocast():
                ...
                loss = mse_loss + self.lambda_sde * sde_loss + self.lambda_smooth * smooth_loss
            ...
            if self.rank == 0:
                train_pbar.set_description(
                    f"train loss: {loss.item():.4f} | mse: {mse_loss.item():.4f} | sde: {sde_loss.item():.4f} | smooth: {smooth_loss.item():.6f} | data time: {data_time_m.avg:.4f}"
                )
            end = time.time()
