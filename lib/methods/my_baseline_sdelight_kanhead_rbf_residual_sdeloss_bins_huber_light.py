...
class MY_BASELINE_SDELIGHT_KANHEAD_RBF_RESIDUAL_SDELOSS_BINS_HUBER_LIGHT(Base_method):
    def __init__(self, args, device, steps_per_epoch):
        ...
        self.huber_criterion = nn.SmoothL1Loss(beta=getattr(args, "huber_delta", 0.03))
        self.criterion = self.mse_criterion

        self.lambda_huber = getattr(args, "lambda_huber", 0.1)
    ...
    def train_one_epoch(self, runner, train_loader, epoch, num_updates, eta=None, **kwargs):
        ...
        for batch_x, batch_y in train_pbar:
            ...
            with self.amp_autocast():
                ...
                huber_loss = self.huber_criterion(pred_y, batch_y)
                sde_loss = self._sde_loss(batch_x, pred_y)
                loss = mse_loss + self.lambda_huber * huber_loss + self.lambda_sde * sde_loss
            ...
            if self.rank == 0:
                train_pbar.set_description(
                    f"train loss: {loss.item():.4f} | mse: {mse_loss.item():.4f} | huber: {huber_loss.item():.4f} | sde: {sde_loss.item():.4f} | data time: {data_time_m.avg:.4f}"
                )
            end = time.time()
