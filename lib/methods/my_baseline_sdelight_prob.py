...
from lib.models import MY_OCEAN_BASELINE_SDELIGHT_PROB
from lib.utils import reduce_tensor
from .base_method import Base_method


class MY_BASELINE_SDELIGHT_PROB(Base_method):
    ...
    def _build_model(self, args):
        return MY_OCEAN_BASELINE_SDELIGHT_PROB(**args).to(self.device)
    ...
    def _predict(self, batch_x, batch_y=None, **kwargs):
        # ВАЖНО: для validation/test OceanVP ждёт только prediction tensor
    ...
    def train_one_epoch(self, runner, train_loader, epoch, num_updates, eta=None, **kwargs):
        ...
        for batch_x, batch_y in train_pbar:
            ...
            if self.rank == 0:
                log_buffer = 'train prob loss: {:.4f}'.format(loss.item())
