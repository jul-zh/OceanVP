...
from lib.models import MY_OCEAN_BASELINE_SDELIGHT_KANHEAD_RBF_RESIDUAL_MULTIKAN
from lib.utils import reduce_tensor
from .base_method import Base_method


class MY_BASELINE_SDELIGHT_KANHEAD_RBF_RESIDUAL_MULTIKAN_SDELOSS_BINS(Base_method):
    ...
    def _build_model(self, args):
        return MY_OCEAN_BASELINE_SDELIGHT_KANHEAD_RBF_RESIDUAL_MULTIKAN(**args).to(self.device)
        ...
        for batch_x, batch_y in train_pbar:
            ...
            if self.rank == 0:
                train_pbar.set_description(
                    f"train loss: {loss.item():.4f} | mse: {mse_loss.item():.4f} | sde: {sde_loss.item():.4f} | data time: {data_time_m.avg:.4f}"
                )
