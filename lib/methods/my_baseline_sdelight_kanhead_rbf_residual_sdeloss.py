...
class MY_BASELINE_SDELIGHT_KANHEAD_RBF_RESIDUAL_SDELOSS(Base_method):
    ...
    def _sde_loss(self, batch_x, pred_y):
        """
        batch_x: [B,T,1,H,W]
        pred_y:  [B,T_out,1,H,W]

        delta_pred ≈ alpha * drift_last + beta * lap(prev_state)
        """
        x_last = batch_x[:, -1]   # [B,1,H,W]
        x_prev = batch_x[:, -2]   # [B,1,H,W]
    ...
    def train_one_epoch(self, runner, train_loader, epoch, num_updates, eta=None, **kwargs):
        ...
        for batch_x, batch_y in train_pbar:
            ...
            if self.rank == 0:
                ...
                log_buffer += ' | mse: {:.4f}'.format(mse_loss.item())
                log_buffer += ' | sde: {:.4f}'.format(sde_loss.item())
