...
class MY_BASELINE_SDELIGHT_KANHEAD_RBF_RESIDUAL_SDELOSS_BINS_TOPK(Base_method):
    def __init__(self, args, device, steps_per_epoch):
        ...
        self.lambda_topk = getattr(args, "lambda_topk", 0.2)
        self.topk_frac = getattr(args, "topk_frac", 0.1)
    ...
    def _topk_mse(self, pred_y, batch_y):
        err2 = (pred_y - batch_y).pow(2).reshape(pred_y.shape[0], -1)
        k = max(1, int(err2.shape[1] * self.topk_frac))
        topk_vals, _ = torch.topk(err2, k=k, dim=1)
    ...
    def train_one_epoch(self, runner, train_loader, epoch, num_updates, eta=None, **kwargs):
        ...
        for batch_x, batch_y in train_pbar:
            ...
            with self.amp_autocast():
                ...
                topk_loss = self._topk_mse(pred_y, batch_y)
                sde_loss = self._sde_loss(batch_x, pred_y)
                loss = mse_loss + self.lambda_topk * topk_loss + self.lambda_sde * sde_loss
            ...
            if self.rank == 0:
                train_pbar.set_description(
                    f"train loss: {loss.item():.4f} | mse: {mse_loss.item():.4f} | topk: {topk_loss.item():.4f} | sde: {sde_loss.item():.4f} | data time: {data_time_m.avg:.4f}"
                )
            end = time.time()
