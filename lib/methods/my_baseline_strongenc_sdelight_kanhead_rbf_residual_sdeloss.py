...
from lib.models import MY_OCEAN_BASELINE_STRONGENC_SDELIGHT_KANHEAD_RBF_RESIDUAL
from lib.utils import reduce_tensor
from .base_method import Base_method


class MY_BASELINE_STRONGENC_SDELIGHT_KANHEAD_RBF_RESIDUAL_SDELOSS(Base_method):
    def __init__(self, args, device, steps_per_epoch):
        ...
        self.sde_alpha = getattr(args, "sde_alpha", 1.0)
        self.sde_beta = getattr(args, "sde_beta", 0.1)

    def _build_model(self, args):
        return MY_OCEAN_BASELINE_STRONGENC_SDELIGHT_KANHEAD_RBF_RESIDUAL(**args).to(self.device)
    ...
    def _sde_loss(self, batch_x, pred_y):
        ...
        for t in range(pred_y.shape[1]):
            ...
            lap = self._laplacian(prev_state)

            rhs = self.sde_alpha * drift_last + self.sde_beta * lap
            total = total + ((delta_pred - rhs) ** 2).mean()
