...
class PiecewiseLinearSplineBasis(nn.Module):
    """
    Simple spline-like basis using triangular hat functions on a fixed knot grid.
    """
...
class KANHeadSpline(nn.Module):
    """
    Pointwise spline KAN head:
    feature at each pixel -> linear proj -> spline basis -> nonlinear mix -> output
    """
    def __init__(
        self,
        in_channels,
...
class SmallDecoderKANHeadSpline(nn.Module):
    def __init__(
        self,
        hidden_dim=32,
        out_steps=16,
...
class MY_OCEAN_BASELINE_KANHEAD_SPLINE(nn.Module):
    def __init__(
        self,
        in_shape,
        hid_S=32,
        ...
        **kwargs,
        ...
        self.decoder = SmallDecoderKANHeadSpline(
            hidden_dim=hid_S,
            out_steps=out_steps,
