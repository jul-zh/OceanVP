...
class KANHeadRBF(nn.Module):
    def __init__(self, in_channels, out_channels, hidden_dim=96, num_centers=48, init_gamma=1.5, dropout=0.05):
        super().__init__()
...
class MY_OCEAN_BASELINE_SDELIGHT_KANHEAD_RBF_RESIDUAL_UV(nn.Module):
    def __init__(
        self,
        in_shape,
        hid_S=32,
        ...
        **kwargs
        ...
        self.encoder = SmallEncoder(in_channels=6, hidden_dim=hid_S)
    ...
    def _build_sde_features(self, x_raw):
        ...
        for t in range(x_raw.shape[1]):
            uv = x_raw[:, t]  # [B,2,H,W]
            drift = torch.zeros_like(uv) if prev is None else uv - prev
            lap = self._laplacian(uv)
            feats.append(torch.cat([uv, drift, lap], dim=1))  # [B,6,H,W]
            prev = uv
