...
class SmallDecoderMultiKAN(nn.Module):
    def __init__(
        self,
        hidden_dim=32,
        out_steps=16,
        ...
        self.kan_smooth = KANHeadRBF(
            in_channels=32,
            out_channels=total_out,
            hidden_dim=kan_head_hidden_dim,
            num_centers=max(16, kan_head_num_centers // 2),
            init_gamma=max(0.5, kan_head_init_gamma * 0.75),
            dropout=kan_head_dropout,
        )
        self.kan_sharp = KANHeadRBF(
            in_channels=32,
            out_channels=total_out,
            hidden_dim=kan_head_hidden_dim,
        ...
        self.alpha_smooth = nn.Parameter(torch.tensor(0.0))
        self.alpha_sharp = nn.Parameter(torch.tensor(0.0))
    ...
    def forward(self, x, skip):
        ...
        out_smooth = self.kan_smooth(x)
        out_sharp = self.kan_sharp(x)

        a1 = torch.sigmoid(self.alpha_smooth)
        a2 = torch.sigmoid(self.alpha_sharp)

        return out_base + a1 * out_smooth + a2 * out_sharp


class MY_OCEAN_BASELINE_SDELIGHT_KANHEAD_RBF_RESIDUAL_MULTIKAN(nn.Module):
    def __init__(
        self,
        in_shape,
        hid_S=32,
        ...
        **kwargs
        ...
        self.decoder = SmallDecoderMultiKAN(
            hidden_dim=hid_S,
