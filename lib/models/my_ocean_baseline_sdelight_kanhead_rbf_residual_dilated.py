...
class DilatedBottleneck(nn.Module):
    def __init__(self, dim=32):
        super().__init__()
        self.branch1 = nn.Conv2d(dim, dim, 3, padding=1, dilation=1)
        self.branch2 = nn.Conv2d(dim, dim, 3, padding=2, dilation=2)
        self.branch3 = nn.Conv2d(dim, dim, 3, padding=3, dilation=3)
        self.norm = nn.GroupNorm(8, dim)
        self.proj = nn.Conv2d(dim * 3, dim, 1)

    def forward(self, x):
        y = torch.cat([
            F.silu(self.branch1(x)),
            F.silu(self.branch2(x)),
            F.silu(self.branch3(x)),
        ], dim=1)
        y = self.proj(y)
        y = self.norm(y)
        return F.silu(y) + x
...
class MY_OCEAN_BASELINE_SDELIGHT_KANHEAD_RBF_RESIDUAL_DILATED(nn.Module):
    def __init__(self, in_shape, hid_S=32, aft_seq_length=16,
        ...
        self.bottleneck = DilatedBottleneck(dim=hid_S)
        self.temporal = TemporalAttentionPool(dim=hid_S)
        self.decoder = DecoderKANResidual(hid_S, self.out_steps, self.out_channels,
