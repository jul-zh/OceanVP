...
class FusionKANGateRBF(nn.Module):
...
class SmallDecoderKANSkipFusion(nn.Module):
    def __init__(
        self,
        hidden_dim=32,
        out_steps=16,
        ...
        self.fusion_gate = FusionKANGateRBF(
            channels=hidden_dim + 16,
            hidden_dim=kan_fusion_hidden_dim,
            num_centers=kan_fusion_num_centers,
            init_gamma=kan_fusion_init_gamma,
            dropout=kan_fusion_dropout,
        )
    ...
    def forward(self, x, skip):
        ...
        x = self.fusion_gate(x)
        x = self.dec1(x)
        return self.out_conv(x)


class MY_OCEAN_BASELINE_KANSKIP_FUSION_RBF(nn.Module):
    def __init__(
        self,
        in_shape,
        hid_S=32,
        ...
        **kwargs,
        ...
        self.decoder = SmallDecoderKANSkipFusion(
            hidden_dim=hid_S,
            out_steps=out_steps,
            out_channels=out_channels,
            kan_fusion_hidden_dim=kan_fusion_hidden_dim,
            kan_fusion_num_centers=kan_fusion_num_centers,
            kan_fusion_init_gamma=kan_fusion_init_gamma,
            kan_fusion_dropout=kan_fusion_dropout,
        )
