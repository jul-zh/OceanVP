...
class SmallDecoderKANHeadPerStepResidual(nn.Module):
    def __init__(
        self,
        hidden_dim=32,
        out_steps=16,
        ...
        self.total_out_channels = out_steps * out_channels

        self.up = nn.Upsample(scale_factor=2, mode="bilinear", align_corners=False)
        self.dec1 = ConvBlock(hidden_dim + 16, 32)

        self.base_head = nn.Conv2d(32, self.total_out_channels, kernel_size=1)
        ...
        # отдельный коэффициент на каждый канал прогноза = каждый future step
        self.alpha_logits = nn.Parameter(torch.zeros(self.total_out_channels))
    ...
    def forward(self, x, skip):
        ...
        alpha = torch.sigmoid(self.alpha_logits).view(1, self.total_out_channels, 1, 1)
        return out_base + alpha * out_kan


class MY_OCEAN_BASELINE_SDELIGHT_KANHEAD_RBF_RESIDUAL_PERSTEP(nn.Module):
    def __init__(
        self,
        in_shape,
        hid_S=32,
        ...
        **kwargs
        ...
        self.decoder = SmallDecoderKANHeadPerStepResidual(
            hidden_dim=hid_S,
